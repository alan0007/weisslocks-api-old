...
if(isset($_REQUEST['company_id']) && isset($_REQUEST['user_id']) &&
    ...
    isset($_REQUEST['is_daily_timeslot'])
)
{
    ...
    if ($is_daily_timeslot === TRUE){
        $day_list = explode(",", $_REQUEST['day_of_week']);
        $day_list =  array_map('intval',$day_list);
        $day_of_week = $day_list; // in array format
    }
    else{
        $day_of_week = NULL;
    }
...
//    }
    ...
    $post_array = array(
        ...
        'date_from' => (String)$date_from, // 2020-07-02
        'date_to' => (String)$date_to,
        'time_from' => (String)$time_from, // 08:05:37
        'time_to' => (String)$time_to,
        'is_daily_timeslot' => (Boolean)$is_daily_timeslot,
        'day_of_week' => $day_of_week, // in array format
    );
...
//        $locksData = $approval_request_for_lock->findOne( $Reg_Query );
        ...
        $category = 'lock_request_normal_access';

...
//        $response['role'] = $role;
    ...
    $cursor_approval_request = $LockRequestApprovalForNormalAccessController->actionGetByCompanyId($_REQUEST['company_id']);
    ...
    if ($cursor_approval_request->count() > 0) {
        foreach ($cursor_approval_request as $approval_request) {
            ...
            if ($is_admin == true) {
                ...
                $response['data'][$i]['is_daily_timeslot'] = $approval_request['is_daily_timeslot'];
                $response['data'][$i]['day_of_week'] = $approval_request['day_of_week'];

                if (isset($approval_request['date_from'])) {
                    $response['data'][$i]['date_from'] = $approval_request['date_from'];
                } else {
                    $response['data'][$i]['date_from'] = '';
                }

                if (isset($approval_request['date_to'])) {
                    $response['data'][$i]['date_to'] = $approval_request['date_to'];
                } else {
                    $response['data'][$i]['date_to'] = '';
                }

                if (isset($approval_request['time_from'])) {
                    $response['data'][$i]['time_from'] = $approval_request['time_from'];
                } else {
                    $response['data'][$i]['time_from'] = '';
                }

                if (isset($approval_request['time_to'])) {
                    $response['data'][$i]['time_to'] = $approval_request['time_to'];
                } else {
                    $response['data'][$i]['time_to'] = '';
                }
                // End Timestamps

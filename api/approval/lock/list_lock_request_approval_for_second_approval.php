...
//        $response['role'] = $role;
    ...
    $cursor_approval_request = $LockRequestApprovalForSecondApprovalController->actionGetByUserIdAndCompanyId($_REQUEST['user_id'],$_REQUEST['company_id']);
...
//            $response['data']['locks'][$i]['valid_until'] = $approval_request['valid_until'];
            $response['data']['locks'][$i]['valid_from'] = $approval_request['valid_from'];
            $response['data']['locks'][$i]['valid_to'] = $approval_request['valid_to'];

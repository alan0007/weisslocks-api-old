...
//        $response['role'] = $role;
    ...
    $cursor_approval_request = $LockRequestApprovalForSpecialAccessController->actionGetByUserIdAndCompanyId($_REQUEST['user_id'],$_REQUEST['company_id']);
...
//            $response['data']['locks'][$i]['valid_until'] = $approval_request['valid_until'];
            ...
            if (isset($approval_request['date'])){
                $response['data']['locks'][$i]['date'] = $approval_request['date'];
            }else{
                $response['data']['locks'][$i]['date'] = '';
            }

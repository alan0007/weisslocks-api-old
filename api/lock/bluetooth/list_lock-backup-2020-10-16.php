                ...
                foreach($accessControl as $accessControl)
                {
                    ...
                    if(is_array($lock_group_id)){
                        if( in_array((int)$accessControl['lock_group_id'], $lock_group_id) ){//If have keygroup
                            ...
                            $response['access_control'][$i]['radius_in_m'] = $accessControl['radious']; //TODO: change in DB
                            $response['access_control'][$i]['added_by'] = $accessControl['added_by'];
                            $response['access_control'][$i]['allowed_days'] = $accessControl['allowed_days'];
                ...
                if ( time() >= strtotime($permit_time_from) && time() <= strtotime($permit_time_to) ){
                    ...
                    if($cursor_locks->count() > 0) {
                        ...
                        foreach($cursor_locks as $locks)
                        {
                            ...
                            $response['locks'][$x]['lock_type'] = $locks['lock_type'];
                            $response['locks'][$x]['lock_model'] = $locks['lock_model'];
                            $response['locks'][$x]['lock_mechanism'] = $locks['lock_mechanism'];
                            $response['locks'][$x]['brand'] = $locks['brand'];
                            $response['locks'][$x]['entrance_visibility'] = $locks['entrance_visibility'];
                            ...
                            $response['locks'][$x]['allowed_days'] = $allowed_days[0];
                            ...
                            if ($approval_for_lock['require_admin_approval'] == true){
                                ...
                                $cursor_approval_request = $collection_approval_request->find( array('lock_id'=>(int)$locks['lock_ID']) );
                                $i=0; // count
...
//                                            'valid_to' => $approval_request['valid_until'],
                                        ...
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['approval_request_for_lock_id'] = $approval_request['approval_request_for_lock_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['company_id'] = $approval_request['company_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['user_id'] = $approval_request['user_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['username'] = $username;
//                                        $response['locks'][$x]['all_previous_approval_request'][$i]['full_name'] = $full_name; // User full name
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['permit_id'] = $approval_request['permit_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['lock_id'] = $approval_request['lock_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['created_timestamp'] = $approval_request['created_timestamp'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['created_by_user_id'] = $approval_request['created_by_user_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['notified_admin_user_id'] = $approval_request['notified_admin_user_id'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_approved'] = $approval_request['admin_approved'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_approved_by'] = $approval_request['admin_approved_by'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_approved_on'] = $approval_request['admin_approved_on'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_rejected'] = $approval_request['admin_rejected'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_rejected_by'] = $approval_request['admin_rejected_by'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['admin_rejected_on'] = $approval_request['admin_rejected_on'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['subadmin_approved'] = $approval_request['subadmin_approved'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['subadmin_approved_by'] = $approval_request['subadmin_approved_by'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['subadmin_approved_on'] = $approval_request['subadmin_approved_on'];
                                        // $response['data'][$i]['valid_from'] = $approval_request['valid_from'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['valid_until'] = $approval_request['valid_until'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['valid_from'] = $approval_request['admin_approved_on'];
//                                        $response['locks'][$x]['all_previous_approval_request'][$i]['valid_to'] = $approval_request['valid_until'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['approval_status'] = $approval_status;

                                        $response['locks'][$x]['all_previous_approval_request'][$i]['from_date'] = $approval_request['from_date'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['to_date'] = $approval_request['to_date'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['from_time'] = $approval_request['from_time'];
                                        $response['locks'][$x]['all_previous_approval_request'][$i]['to_time'] = $approval_request['to_time'];
                                    }
                                }

                                $response['locks'][$x]['last_approval_request'] = $current_user_approval_request;
                                if ( strtotime($datetime_now) >= strtotime($current_user_approval_request['valid_from']) && strtotime($datetime_now) <= strtotime($current_user_approval_request['valid_to']) ) {
                                    ...
                                    $response['locks'][$x]['last_approval_request']['open_lock'] = true;
                                }
                                else{
                                    //$response['locks'][$x]['last_approval_request']['valid'] = false;
                                    $response['locks'][$x]['last_approval_request']['open_lock'] = false;
                                }
                            }

...
require dirname(dirname(__FILE__)).'/modules/v1/log/controllers/LogLockActivityController.php';
...
if( isset($_REQUEST['lock_id']) && isset($_REQUEST['user_id']) )
{
    ...
    $LogActivityController = new LogLockActivityController($Database);

    $result = $LogActivityController->actionGetByUserIdAndLockId($_REQUEST['user_id'],$_REQUEST['lock_id']);
    if( $result->count() > 0){

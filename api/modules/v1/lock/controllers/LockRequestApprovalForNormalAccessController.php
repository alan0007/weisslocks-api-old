...
include(dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))).'/common/v1/models/LockRequestApprovalForNormalAccess.php');

use common\config\Database;
use common\v1\models\LockRequestApprovalForNormalAccess;
use MongoCursorException;

class LockRequestApprovalForNormalAccessController
{

    public $modelClass = 'common\v1\models\LockRequestApprovalForNormalAccess';
    public $Model;
    public $Database;
    private $server_name;
    private $app_data;
    ...
    {
        $this->Model = new LockRequestApprovalForNormalAccess();
    ...
    public function actionInsert($post_array)
    {
        $collection = $this->Database->getCollectionPhp5($this->app_data,$this->Model->tableName());
        $post = array(
            'id'  => getNext_users_Sequence('lock_request_approval_for_normal_access'),
            ...
            'date_from' => (String)$post_array['date_from'], // 2020-07-02
            'date_to' => (String)$post_array['date_to'],
            'time_from' => (String)$post_array['time_from'], // 08:05:37
            'time_to' => (String)$post_array['time_to'],
            'is_daily_timeslot' => (Boolean)$post_array['is_daily_timeslot'],
            'day_of_week' => $post_array['day_of_week'],
        );
...
//            $locksData = $collection->findOne( $Reg_Query );
    ...
    public function actionUpdateById($id,$post_array){
        ...
        $post = array(
            ...
            'time_to' => (String)$post_array['time_to'],
            'is_daily_timeslot' => (Boolean)$post_array['is_daily_timeslot'],
            'day_of_week' => json_encode( $post_array['day_of_week'] ),
        );
        try{

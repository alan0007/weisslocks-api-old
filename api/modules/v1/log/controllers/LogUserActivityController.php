...
include(dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))).'/common/v1/models/LogUserActivity.php');

use common\config\Database;
use common\v1\models\LogUserActivity;

class LogUserActivityController
{
    ...
    {
        $this->Model = new LogUserActivity();
    ...
    public function actionInsert($company_id,$user_id,$description,$category,$datetime)
    {
        $collection = $this->Database->getCollectionPhp5($this->app_data,$this->Model->tableName());
        $post = array(
            'log_activity_id' => $this->Database->getNext_users_Sequence($this->table_name),

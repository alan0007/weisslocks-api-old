...
if($_REQUEST['action'] == 'add' && isset($_REQUEST['user_name']) )
{
    ...
    //$cursor = $collection->find( $Reg_Query );
		...
		$permit = $app_data->permit_to_enter;
		$post = array(
...
else if($_REQUEST['action'] == 'view' && isset($_REQUEST['permit_id']) && $_REQUEST['permit_id'] != '')
{
	$response['status'] = 'false';
	if($_REQUEST['permit_id'] == 1)
	{
		$collection = new MongoCollection($app_data, 'permit_to_enter');
		$permit = $collection->find();
		
		if($permit->count() > 0) { $response['status'] = 'true';
			foreach($permit as $permit)
			{
				unset($permit['_id']);
				$permit['company_name'] = '';
				if( !empty( $permit['company_id'] ) &&  $permit['company_id'] != 0 )
				{
					$collection_com = new MongoCollection($app_data, 'company');
					$coms = $collection_com->findOne(array('company_ID'=>(int)$permit['company_id']));
					if(isset($coms['company_ID']))
					{
						$permit['company_name'] = $coms['company_name'];
						$permit['company_ref_id'] = $coms['company_ref'];
					}
				}
				
				if($permit['user_id'] == $_REQUEST['user_id'])
				{
					$response['current_user'][] = $permit;
				}
				if($permit['user_id'] != $_REQUEST['user_id'])
				{
					
					$start_date = new DateTime( date('d-m-Y H:i',strtotime( $permit['registered_time'] )) );
					$since_start = $start_date->diff(new DateTime( date('d-m-Y H:i') ));
					
					/*echo date('d F Y, H:i') . '---' . $permit['registered_time'] . '-----';
					...
					{
						$permit['duration'] = date('H:i', strtotime( $permit['registered_time']));
						//echo 'Before 1 day<br><br>';
					}
					else if($since_start->days == 1)
					{
						$permit['duration'] = 'Yesterday';
						//echo 'Yesterday<br><br>';
					}
					else if($since_start->days >= 2)
					{
						$permit['duration'] = date('d/m', strtotime( $permit['registered_time']));
						//echo  date('d/m', strtotime( $permit['registered_time'])) . ' <br><br>';
					}
					else { $permit['duration'] = '--/--'; }

...
//    $collection = $UserController->actionGetUserCollection();
    ...
    {
        $update_udid = $LoginController->actionUpdateUdid($cursor['user_id'],$cursor['role'],$cursor['UDID_IOS']);
        $response['udid_updated'] = $update_udid;

        $response['status'] = 'true';
        unset($response['error']);

        $update_last_login = $LoginController->actionUpdateLastLogin($cursor['user_id']);

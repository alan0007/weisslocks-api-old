...
if($_REQUEST['action'] == 'add' && isset($_REQUEST['user_name']) && isset($_REQUEST['user_email']) && isset($_REQUEST['user_role']))
{
	...
	if ( $_REQUEST['user_registration_image_name'] == null || $_REQUEST['user_registration_image_name'] == ""){
    ...
    else
    {
		...
		if ( $_REQUEST['user_registration_image_name'] || $_REQUEST['user_registration_image_name'] == ""){
			$_REQUEST['user_registration_image_name'] = "no_uploaded_image.png";
		}
		...
		$post = array(
			...
			'residence'  => $_REQUEST['residence'], //Residence-Yes/No
...
else if($_REQUEST['action'] == 'login' && isset($_REQUEST['username']) && isset($_REQUEST['password']))
{
	...
	{
			if(in_array($cursor['role'],array(2,3,4,5,6,7,8)))//Default is 2,3
			{
...
else if($_REQUEST['action'] == 'login_facial' && isset($_REQUEST['username']))
{
	$response['status'] = 'false';
	$response['error'] = 'Invalid Credentials';
	$collection = new MongoCollection($app_data, 'users');
	$Login_Query = array('username' => $_REQUEST['username']);
	$cursor = $collection->findOne( $Login_Query );
	if(isset($cursor['user_id']))
	{
			if(in_array($cursor['role'],array(2,3,4,5,6,7,8)))//Default is 2,3
		...
		$users =  json_decode($cursor['user_id']);
		 for($i=0;$i<count($users);$i++)
		 {
			...
			$cursor_users = $collection1->findOne( array('user_id'=>(int) $users[$i] ) );
			 if(isset($cursor_users['user_id']) && in_array($cursor_users['role'],array(4,5,6,7)))
			 {
				 $response['status'] = 'true';

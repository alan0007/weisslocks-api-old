...
if(isset($_REQUEST['process']) && $_REQUEST['process'] == 'Save')
{
	...
	if($_REQUEST['id'] == 0)
	{
		$keygroup = $LockRequestApprovalForNormalAccessController->actionInsert($post_array);
        $post_array = array(
		...
		if($LockRequestApprovalForNormalAccessController->actionInsert($post_array) === TRUE){
            $update_success = TRUE;
            echo "<script>window.location='lock_request_approval_for_normal_access.php?success=true'</script>";
        }
        else{
            echo "<script>window.location='lock_request_approval_for_normal_access.php?success=false'</script>";
        }
	}
	else
	{
        if($LockRequestApprovalForNormalAccessController->actionUpdateById($_REQUEST['id'],$post_array) === TRUE){
            $update_success = TRUE;
            echo "<script>window.location='lock_request_approval_for_normal_access.php?success=true'</script>";
        }
        else{
    ...
    <div id="wrapper">
       <?php include("menu.php");?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Request Approval for Normal Access</h1>
                </div>
                <div class="col-lg-6">

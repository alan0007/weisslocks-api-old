...
require dirname(__FILE__).'/api/modules/v1/lock/controllers/LockRequestApprovalForSpecialAccessController.php';
...
{
    $delete = $LockRequestApprovalForSpecialAccessController->actionDeleteById($_REQUEST['id'] );
    if ($delete === TRUE){
...
//If superadmin
if($company_id == 0){
    $data = $LockRequestApprovalForSpecialAccessController->actionIndex();
}else{
    $data = $LockRequestApprovalForSpecialAccessController->actionGetByCompanyId($company_id);
}
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            ...
            <div class="col-lg-12">
                <div style="width:150px;clear:both;padding-left:30px;"><a class="btn btn-lg btn-primary btn-block" href="lock_request_approval_for_special_access_manage.php"> Add </a></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            if($data->count() > 0)
                            {
                                ...
                                { ?>
                                    <tr>
                                        <td><?php echo $lock['approval_request_for_lock_id']; ?></td>
                                        <td><?php
                                            $company_found = $CompanyController->actionGetOneById($lock['company_id']);
                                        ...
                                        <td><?php echo $lock['date']; ?></td>
                                        <td><?php echo $lock['time_from']; ?></td>
                                        <td><?php echo $lock['time_to']; ?></td>
                                        <td>
                                            <a href="lock_request_approval_for_special_access.php?id=<?php echo $lock['id']; ?>">  Edit </a>
                                            <a onclick="return confirm('Are you sure?')" href="lock_request_approval_for_special_access.php?delete=record&id=<?php echo $lock['id']; ?>">  Delete </a>
                                        </td>
                                    </tr>
                                    <?php
                                }

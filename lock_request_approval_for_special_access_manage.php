...
if(isset($_REQUEST['process']) && $_REQUEST['process'] == 'Save')
{
    ...
    if($_REQUEST['id'] == 0)
    {
        $keygroup = $LockRequestApprovalForSpecialAccessController->actionInsert($post_array);
        $post_array = array(
        ...
        if($LockRequestApprovalForSpecialAccessController->actionInsert($post_array) === TRUE){
            $update_success = TRUE;
            echo "<script>window.location='lock_request_approval_for_special_access.php?success=true'</script>";
        }
        else{
            echo "<script>window.location='lock_request_approval_for_special_access.php?success=false'</script>";
        }
    }
    else
    {
        if($LockRequestApprovalForSpecialAccessController->actionUpdateById($_REQUEST['id'],$post_array) === TRUE){
            $update_success = TRUE;
            echo "<script>window.location='lock_request_approval_for_special_access.php?success=true'</script>";
        }
        else{
    ...
    <div id="wrapper">
       <?php include("menu.php");?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Request Approval for Second Approval</h1>
                </div>
                <div class="col-lg-6">

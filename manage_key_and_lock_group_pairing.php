...
if(isset($_REQUEST['process']) && $_REQUEST['process'] == 'Save')
{
    ...
    $allowed_days = isset($_REQUEST['allowed_days']) ? $_REQUEST['allowed_days'] : array();
    if (isset($_REQUEST['allowed_days'])){
	...
	*/
	...
	if($_REQUEST['keyLockGroup_ID'] == 0)
	{
			...
			$post = array(
                ...
                'is_timestamp_continuous' => $_REQUEST['is_timestamp_continuous']
				);
									...
									if(isset($_REQUEST['keyLockGroup_ID']))
									{
										...
										if($cursor->count() > 0)
										{
											foreach($cursor as $KeyLockGroup)
											{
                                                ...
                                                if (isset($KeyLockGroup['is_timestamp_continuous'])){
                                                    $is_timestamp_continuous = $KeyLockGroup['is_timestamp_continuous'];
                                                }
											}
										}
									}
...
//														$companies = $collection->find();
												...
												<?php } ?>
												<div class="form-group">
													<label>Access Control User List: </label>
													<div class="users">
													<?php
													 $collection = new MongoCollection($app_data, 'company');
													 $cursor = $CompanyController->actionGetOneById($company_id);
													 if(isset($cursor['company_ID']))
													 {
                                                         $user_list = $cursor['user_id'];
														 $users =  json_decode($cursor['user_id'],TRUE);

														 foreach ($users as $user_id){
                                                             $cursor_users = $UserController->actionGetOneById($user_id);
                                                             if(isset($cursor_users['user_id']) && in_array($cursor_users['role'],array(2,3,4,5,6,7,8)))
                                                             { ?>
                                                                 <input id="user_<?php echo $user_id; ?>" <?php echo in_array( $cursor_users['user_id'] , $selected_users) ? 'checked' : ''; ?> type="checkbox" name="users[]" value="<?php echo $cursor_users['user_id']; ?>" />
                                                                 <label for="user_<?php echo $user_id; ?>"><?php echo $cursor_users['username']; ?></label><br/>
                                                                 <?php
                                                             }
                                                         }
...
//                                                             $cursor_users = $UserController->actionGetOneById($users[$i]);
//															 if(isset($cursor_users['user_id']) && in_array($cursor_users['role'],array(2,3,4,5,6,7,8)))
//															 { ?>
...
//														 }
                                        ...
                                        <div class="form-group">
                                            <label> Is the Timestamp Continous (Select No if is time slot) </label>
                                            <select name="is_timestamp_continuous" class="form-control">
                                                <option <?php echo $is_timestamp_continuous == 1 ? 'selected="selected"' : ''; ?> value="1"> Yes </option>
                                                <option <?php echo $is_timestamp_continuous == 0 ? 'selected="selected"' : ''; ?> value="0"> No </option>
                                            </select>
                                        </div>
